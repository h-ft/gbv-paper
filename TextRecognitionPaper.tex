...
\vspace{2cm}
\begin{abstract}
Scene text detection methods has been proven to be reliable on detecting scanned images. However, they usually fall short when it comes to natural scene images, which introduced many random factors in the equation. In this paper, the scene text detector EAST (abbreviation for Efficient and Accurate Scene Text Detector) will be tested on natural scene images of varying difficulty from a self-made evaluation dataset of 640p square images. Experiments on scanned images shows promising results in that almost every text is detected with a few exceptions. On natural scene images, the results vary from almost every text detected to almost none.
\end{abstract}
	\newpage
	\tableofcontents
	\newpage
